	...
	public AddStudentPane() {
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent me) {
				if (me.getClickCount() == 2) { // to detect doble click events
					...
					int index = getIndexOf(allModules, listModules.get(row));// get index of module in the searched
																				// modules liste

					selectedModules.add(allModules.get(index));
					allModules.remove(index);
					table.repaint();
					filterBtn.doClick();
		...
		filterBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				listModules.clear();
				listModules.addAll(searchManager.filterModulesByNiveau(niveau, allModules));
		...
		saveEtu.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (filled == false) {
					...
					for (int i = 0; i < selectedModules.size(); i++) {
						InscriptionModule inscModule = new InscriptionModule(selectedModules.get(i), null, null,
								inscPedago, 0);
						inscPedago.addInscriptionModule(inscModule);
					}
					etd.addInscrPedago(inscPedago);
					etd.addInscrAdmin(
							new InscriptionAdministrative(niveau, etd, Integer.parseInt(annee), -1, -1, -1, -1));
					try {
...
//						inscriptionManager.inscrirPedago(etd, selectedModules, Integer.parseInt(annee));
	...
	public static int getIndexOf(List list, Object value) {
